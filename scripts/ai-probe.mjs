 ...
 *
 * ## Две ошибки, которые здесь были
 *
 * Первая: вес брался как `threshold.bad` — а это КИЛОБАЙТЫ ПРИПИСКИ, из
 * которых base64 делает тело в треть длиннее. Контрольные шаги уходили с
 * телом 26 КБ при пороге в 35 — то есть заведомо проходящим весом. Они
 * проверяли не то и отвечали «всё хорошо» на вопрос, которого никто не
 * задавал. Теперь вес задаётся в байтах ТЕЛА и берётся с запасом вдвое.
 *
 * Вторая: «прокси с негодным секретом». Воркер проверяет секрет ПЕРЕД
 * чтением тела и отвечает 403 сразу — байты до Hetzner так и не поедут, а
 * быстрый ответ выглядит как «доехало». Ровно это и вышло: 0,2 секунды.
 * Теперь секрет настоящий, а быстрый отказ обеспечивается несуществующей
 * моделью: воркер обязан прочитать тело целиком и переслать его, Anthropic
 * отвечает 404 мгновенно. Тело при этом проходит весь путь.
 */
if (firstFailure) {
  console.log("\nЧАСТЬ 2. Тот же вес, но другие адресаты: чей это порог\n");

  // Вдвое выше порога — чтобы вопрос «а достаточно ли тяжело» не возникал.
  const HEAVY_KB = Number(process.env.PROBE_HEAVY_KB || Math.max(64, Math.round((threshold.bad || 48) * 1.4 * 2)));

  // Куда вообще ведёт адрес прокси. Если он окажется не там, где мы думаем
  // (например, за чужим CDN), это меняет весь разбор — а стоит одну строку.
  try {
    const { lookup } = await import("node:dns/promises");
    const addresses = await lookup(new URL(BASE).hostname, { all: true });
    console.log(`   ${new URL(BASE).hostname} → ${addresses.map((a) => a.address).join(", ")}\n`);
  } catch {
    // Резолвер недоступен — не повод останавливать проверку.
  }

  await probe("2a. полный путь, модель-заглушка", {
    body: {
      model: "no-such-model-probe",
      max_tokens: 16,
      messages: [{ role: "user", content: "x".repeat(HEAVY_KB * 1024) }],
    },
    timeoutMs: SWEEP_TIMEOUT_MS,
    // Модели такой нет, и Anthropic отвечает отказом мгновенно. Но чтобы
    // ответить, он должен сперва получить тело — то есть весь путь
    // приложение → Caddy → воркер → Anthropic обязан пронести эти байты.
    // Быстрый отказ здесь означает «вес проходит, дело в чём-то другом».
    expect: true,
  });
  ...
  await probe("2c. посторонний хост, вчетверо тяжелее", {
    url: "https://httpbin.org/post",
    headers: { authorization: undefined },
    body: ballast(HEAVY_KB * 4),
    timeoutMs: SWEEP_TIMEOUT_MS,
    // Есть ли у канала наружу свой порог, просто выше нашего. Если 2b
    // прошёл, а этот нет — уплотнять снимок бессмысленно, лечить надо
    // направление: пусть Anthropic скачивает картинку сам.
    expect: true,
  });

  /**
   * Обход, который мы на этих замерах и строим (lib/ai/photo-link.ts):
   * картинку не отправляем, а даём на неё ссылку. Тело запроса —
   * полтора килобайта, порог веса обойдён.
   *
   * Но у обхода есть своё условие: серверы Anthropic должны суметь зайти
   * на наш домен. Направление входящее и заведомо живое — сайт открывается
   * из любой страны, — однако проверить это надо ДО того, как радоваться.
   * Берём открытую картинку сайта: своего кода для проверки не нужно, а
   * ответ модели покажет и доступность, и что зрение по ссылке работает.
   */
  const SITE = (process.env.SITE_URL || "https://jivoetelo.ru").replace(/\/+$/, "");
  await probe("2d. картинка ссылкой на наш домен", {
    body: {
      model: VISION,
      max_tokens: 100,
      messages: [{
        role: "user",
        content: [
          { type: "image", source: { type: "url", url: `${SITE}/og.jpg` } },
          { type: "text", text: "Опиши картинку одним предложением." },
        ],
      }],
    },
    timeoutMs: SWEEP_TIMEOUT_MS,
...
if (!firstFailure) {
  ...
  console.log("  2a доехал            → вес проходит; виновата не сеть, а сам разбор;");
  console.log("  2a встал, 2b доехал  → порог у направления на прокси, а не у канала.");
  console.log("                         Значит участок Россия → Hetzner или сам Caddy;");
  console.log("  2b встал тоже        → канал наружу у этого VPS целиком. Кодом не лечится;");
  console.log("  2b доехал, 2c нет    → у канала свой порог, просто выше. Сжимать снимок");
  console.log("                         бессмысленно — надо менять направление передачи.");
}
