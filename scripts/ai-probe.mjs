 ...
 * заданного размера. Если виснет на определённом объёме — дело в пути до
 * прокси, а не в самом снимке.
 */
...
// Развёртка по размеру: та же картинка, разное тело запроса. Ищем порог.
for (const kb of [16, 64, 128, 256]) {
  steps.push([`3.${kb}. картинка 64×64, тело ${kb} КБ`, {
    model: VISION, max_tokens: 200, messages: [image(paddedImage(kb))],
  }]);
}
...
if (!firstFailure) {
  ...
  console.log("  3.N — дело в РАЗМЕРЕ ТЕЛА запроса, а не в картинке:");
  console.log("        картинка везде одна и та же, 64×64, меняется только вес.");
  console.log("        Значит виноват путь до прокси или сам прокси, а не модель.");
