    ...
    for (const width of VIEWPORTS) {
      ...
      for (const path of paths) {
        ...
        const found = await page.evaluate((minTap) => {
          ...
          const inlineHost = "p, li, figcaption, h1, h2, h3, td, th, small, label, dt, dd, span";
          for (const el of document.querySelectorAll("a, button, input, select, summary")) {
            ...
            // Флажок и радио внутри метки: нажимается вся метка, а сам
            // квадрат 24×24 — ровно минимум WCAG 2.5.8 уровня AA.
