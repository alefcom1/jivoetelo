...
const SHOTS = [
  ...
  {
    src: "docs/screenshots/site-cabinet.png",
    out: "cabinet-desk.webp",
    /**
     * Тот же кабинет, но для блока «Как это выглядит» на главной.
     *
     * Без шапки и без пустых полей. В кадре целиком по 345 CSS-точек с каждой
     * стороны занимал фон, а сверху шло меню — вместе это две трети снимка,
     * которые на лендинге не рассказывают ничего. Границы найдены по самому
     * изображению: контентная колонка идёт с 522-й точки по 2357-ю, шапка
     * кончается на 126-й.
     *
     * Высота обрезана по промежутку между карточками, чтобы снимок кончался
     * целой записью, а не половиной следующей. Промежутки в этом кадре:
     * 1272–1291 (после обеда), 1490–1509 (после перекуса), 1708–1751 (после
     * ужина). Режем по последнему — в кадр входят все четыре приёма пищи.
     * Прежние 1282 обрывали список на обеде, и дневник выглядел пустее, чем
     * он есть. Если экран «Сегодня» перевёрстывают, числа надо снять заново.
     */
    crop: { left: 480, top: 126, width: 1919, height: 1730 },
    width: 1500,
  },
  {
