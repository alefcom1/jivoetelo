/**
 * Знак сервиса. Раньше в шапке и подвале стояла буква «Ж» — монограмма
 * заголовочным шрифтом; теперь тот же знак, что и в фавиконе, чтобы вкладка
 * браузера и страница показывали одно и то же.
 *
 * Разметка вставляется прямо в страницу, а не через <img src="/favicon.svg">:
 * так знак не требует отдельного запроса, не мигает при загрузке и не зависит
 * от кэша браузера на favicon.
 *
 * Тот же файл лежит в public/favicon.svg — при замене знака менять оба.
 */
export function Logo({ size = 27 }: { size?: number }) {
  return <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    role="img"
    aria-label="Живое Тело"
  >
    <path
      d="M22 19.2727C22 20.779 20.779 22 19.2727 22H14.7273C13.221 22 12 20.779 12 19.2727V12H19.2727C20.779 12 22 13.221 22 14.7273V19.2727Z"
      fill="#68C4FF"
    />
    <path
      d="M20 2C21.1046 2 22 2.89543 22 4V7C22 8.10457 21.1046 9 20 9H17C15.8954 9 15 8.10457 15 7V4C15 2.89543 15.8954 2 17 2H20Z"
      fill="#0C79D8"
    />
    <path
      d="M7 15C8.10457 15 9 15.8954 9 17V20C9 21.1046 8.10457 22 7 22H4C2.89543 22 2 21.1046 2 20V17C2 15.8954 2.89543 15 4 15H7Z"
      fill="#0C79D8"
    />
    <path
      d="M12 12H4.72727C3.22104 12 2 10.779 2 9.27273V4.72727C2 3.22104 3.22104 2 4.72727 2H9.27273C10.779 2 12 3.22104 12 4.72727V12Z"
      fill="#2E9EFF"
    />
  </svg>;
}
