...
export default function Home() {
  ...
  return <main>
    ...
    <section className="journal" id="journal"><div className="section-top"><p className="kicker">Журнал <i /></p><div><h2>О теле —<br /><em>с уважением.</em></h2><p>Как устроен сервис и почему он считает именно так — без давления и обещаний «минус десять к лету».</p></div></div><div className="articles">{featuredArticles().map((article) => <article key={article.slug}><span className="articles-hero" aria-hidden><ArticleHero slug={article.slug} image={article.heroImage} alt="" /></span><span>{article.kicker.toUpperCase()} · {article.minutes} МИН</span><h3>{article.titleShort}</h3><p>{article.description}</p><Link href={`/blog/${article.slug}`}>Читать статью →</Link></article>)}</div><p className="journal-all"><Link href="/blog">Все статьи журнала →</Link></p></section>
