...
export default async function ClientsPage() {
  ...
  if (!specialist) {
    ...
    if (!profile) {
      return (
        <main className="pro-cab-gate">
          <h1>Кабинет специалиста</h1>
          <p>
            Кабинет бесплатный и открывается сразу — ждать ответа не нужно. Четыре поля,
            обязательное одно.
          </p>
    ...
    if (profile.status === "pending") {
      // Остаток прежнего, ручного пути: строка заведена, но работать ещё не
      // пускали. Теперь такому человеку незачем ждать — он может открыть
      // кабинет сам, той же формой.
      return (
        <main className="pro-cab-gate">
          <h1>Кабинет специалиста</h1>
          <p>
            Ваша заявка у нас есть, но ждать её разбора больше не нужно: кабинет теперь
            открывается сразу. Заполните профиль — и можно приглашать клиентов.
          </p>
          <Link className="black-button" href="/pro/registraciya">Открыть кабинет <b>↗</b></Link>
        </main>
      );
    ...
    // rejected или suspended — доступ закрыт решением человека, и восстановить
    // его формой регистрации нельзя: действие проверяет статус отдельно.
    return (
      <main className="pro-cab-gate">
        <h1>Кабинет специалиста</h1>
        <p>Доступ к разделу сейчас закрыт. Если это ошибка, ответьте на наше письмо — разберёмся.</p>
      </main>
    );
  }
