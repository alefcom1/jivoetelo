...
/**
 * Давали ли уже доступ к камере в этом окне.
 *
 * ## Почему модульная переменная, а не localStorage
 *
 * Она в точности отражает то, что нам нужно знать: живо ли разрешение,
 * выданное браузером. В Telegram на iOS доступ к камере действует на время
 * жизни окна WebView — закрыли мини-приложение, и разрешение ушло вместе с
 * ним. Модульная переменная живёт ровно столько же и обнуляется вместе с
 * ним, а localStorage пережил бы сессию и обещал бы разрешение, которого
 * больше нет.
 *
 * Зачем это нужно: пока разрешения нет, видоискатель не включается сам.
 * Иначе окно «разрешить камеру?» выскакивает при каждом заходе на вкладку —
 * в том числе когда человек зашёл сюда ради «Повторить» и снимать не
 * собирался. После первого разрешения поток запускается сам: окна больше не
 * будет, и лишнее касание становится лишним.
 */
let grantedThisSession = false;

export function cameraGrantedThisSession(): boolean {
...
export function useCamera() {
  ...
  const attach = useCallback((stream: MediaStream, list: CameraDevice[]) => {
    // Поток пошёл — значит разрешение выдано и живо до конца этой сессии.
