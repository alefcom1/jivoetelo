...
// Диагностика сканера штрихкодов. Открывать внутри Telegram по адресу
// /tg/scan-test.
//
// Заводилась как разовая — чтобы решить, брать ли `showScanQrPopup`. Решение
// принято и без неё: этот метод официально описан только для QR, в Desktop-
// и Web-клиентах не работает вовсе (docs/market-research.md, раздел 8), а
// читает ли он EAN-13 — вопрос, на который документация не отвечает. Сканер
// сделан на `BarcodeDetector` поверх нашего же видоискателя
// (app/use-barcode-scan.ts).
//
// Страница осталась, но отвечает теперь на другой вопрос: есть ли
// BarcodeDetector в вебвью конкретного телефона. Это ровно то, что нужно
// узнать, когда человек пишет «сканер не работает», — и узнать это иначе
// нельзя. В навигацию она не добавлена и в поиск не попадает: `/tg` целиком
// закрыт в robots.ts.
