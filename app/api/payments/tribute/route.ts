 ...
 * рассказывает о настройках — кроме одного: настроен ли приём оплаты вообще.
 * Это не секрет (страница тарифа и так это показывает), а вот в момент
 * «тестовый запрос не проходит» ответ на этот вопрос экономит час.
 */
export async function GET() {
  return Response.json(
    { ok: true, provider: "tribute", configured: getTributeConfig() !== null },
    { status: 200 },
  );
}
