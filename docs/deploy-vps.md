...
getent hosts jivoetelo.ru
getent hosts www.jivoetelo.ru
```

Адреса должны совпасть. Если в ответе стоит что-то вроде `31.31.198.90` и
проверка потом упирается в `dnsadmin.hosting.reg.ru` — домен ещё висит на
парковке регистратора: A-записи меняются в панели reg.ru, изменения доходят
