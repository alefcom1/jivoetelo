...
2. **Автодеплой через GitHub Actions.** Workflow запускается на серверах
   GitHub (не в контейнере ассистента), подключается к VPS по SSH из
   секретов репозитория и выполняет `git pull`, сборку и миграции. После
   настройки каждый push в ветку выкатывается сам. Понадобится: адрес
   сервера, пользователь, приватный ключ (отдельный, только для деплоя) —
   всё в Secrets репозитория, в код ничего не попадает.

Второй способ убирает ассистента из цепочки целиком; если он нужен, скажите
— напишу workflow.
