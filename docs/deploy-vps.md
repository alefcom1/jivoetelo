...
Свежий бот от @BotFather не делает ничего: он не знает ни куда слать
сообщения, ни что у него есть приложение. Выглядит это как «бот пустой,
ничего не отвечает». Нужны четыре шага, все обязательные.

**1. Токен в `.env`.** `TELEGRAM_BOT_TOKEN` от @BotFather. Без него
`/api/tg/*` отвечает 503 — это защита от запуска без проверки подписи, а не
ошибка.

**2. Секрет вебхука в `.env`.** `TELEGRAM_WEBHOOK_SECRET`, сгенерировать
`openssl rand -hex 32`. Без него маршрут вебхука тоже отвечает 503.
После правки `.env` — перезапустить приложение, иначе оно читает старые
значения: `docker compose up -d`.

**3. Привязать домен к боту.** В @BotFather: `/setdomain` → выбрать бота →
`https://jivoetelo.ru`. Без этого Telegram отклоняет любую web_app-кнопку,
и приложение неоткуда открыть. Отдельно `/newapp` нужен, только если
хочется прямую ссылку вида `t.me/бот/app`.

**4. Зарегистрировать вебхук, команды и кнопку:**

```bash
node scripts/webhook.mjs set
```

Одна команда делает три вещи: регистрирует вебхук (иначе бот не получает ни
одного сообщения), задаёт команды `/start` и `/stop` и ставит синюю кнопку
«Дневник», которая открывает Mini App. Если шаг 3 пропущен, скрипт про это
скажет прямым текстом и остальное всё равно настроит.

Проверка — `node scripts/webhook.mjs` без аргументов: показывает
зарегистрированный URL, очередь недоставленных апдейтов и последнюю ошибку
доставки с расшифровкой, что чинить.
