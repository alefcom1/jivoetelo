...
/**
 * Вопросы и ответы статьи — разметкой FAQPage.
 *
 * ## Зачем это, если ответы и так в тексте
 *
 * Часть запросов — это вопросы целиком: «насколько точен ИИ», «бывает ли
 * бесплатно». Поиск умеет показывать на такие запросы готовый ответ прямо в
 * выдаче, но только если ответ размечен. Без разметки страница участвует в
 * общем зачёте ссылок, с разметкой — ещё и в блоке ответов, куда ссылок
 * попадает вчетверо меньше.
 *
 * ## Правило, которое нельзя нарушать
 *
 * В разметку идёт **только то, что действительно есть на странице теми же
 * словами**. Разметка, обещающая ответ, которого в тексте нет, — это обман
 * поиска, и наказывают за него всю страницу целиком. Поэтому пары
 * собираются из самих разделов статьи, а не пишутся отдельно «для поиска».
 */
