...
/**
 * Предел ожидания ответа модели и число повторов.
 *
 * У SDK по умолчанию десять минут и две попытки — то есть полчаса в худшем
 * случае. Для фонового задания это разумно, для человека, который стоит над
 * тарелкой и смотрит на «Разбираем…», — нет: столько никто не ждёт, а
 * отменить нечем. На iPhone запрос в итоге убивала сама система примерно
 * через три минуты, и наружу это выходило чужой английской строкой
 * «Load failed».
 *
 * Сорок секунд с запасом перекрывают всё нормальное: разбор текста идёт
 * секунды, снимок — до десятка. Одна повторная попытка оставлена ради
 * случайных обрывов на пути к прокси; больше — только растягивает ожидание.
 * Клиентский предел (ANALYZE_TIMEOUT_MS в app/tg/api.ts) заведомо больше
 * суммы, чтобы человек увидел настоящую ошибку сервера, а не свою.
 */
...
export function createAnthropicClient(): Anthropic {
  ...
  return new Anthropic({ fetch: upstreamFetch, timeout: REQUEST_TIMEOUT_MS, maxRetries: MAX_RETRIES });
}
