...
- Различай два вида вопросов. Если вопрос ДОБАВЛЯЕТ ингредиент («была ли заправка?»), refinesIndex не ставь. Если вопрос УТОЧНЯЕТ уже найденную позицию («какой именно йогурт?», «на каком масле жарили?»), обязательно поставь refinesIndex — номер этой позиции в items, считая с нуля. Тогда выбранный вариант заменит её; без refinesIndex он допишется рядом, и в приёме пищи окажется две одинаковые позиции.
