...
test("в Метрику не уходит ничего, кроме имени цели", async () => {
  ...
  for (const goal of ALL_GOALS) {
    // Цифры разрешены: события воронки называются day2_return и
    // week1_active, и это по-прежнему просто имя, а не данные о человеке.
    assert.match(goal, /^[a-z][a-z0-9_]*$/, `${goal}: идентификатор цели должен быть простым именем`);
  }
