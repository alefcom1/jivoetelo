...
test("список статусов закрыт, и «одобрено» в нём одно", () => {
  // Четвёртым стал `offered` — предложение модератора, на которое автор ещё
  // не ответил. Он стоит первым намеренно: это состояние ДО очереди
  // модерации, а не рядом с ней. Список проверяется целиком, потому что от
  // него зависит, что показывается публично: лишний статус, забытый в
  // выборке `approvedPhotosFor`, — это чужой снимок на витрине.
  assert.deepEqual([...PHOTO_STATUSES], ["offered", "pending", "approved", "rejected"]);
  assert.equal(PHOTO_STATUSES.filter((status) => status === "approved").length, 1);
});
