...
test("настоящая очередь читается и не пуста", () => {
  ...
  // Глоссарий и методологию здесь не требуем: их разделы могут быть
  // выполнены целиком (в августе 2026 так и вышло — термины написаны разом,
  // см. примечание в очереди). Что парсер понимает эти разделы, проверяет
  // синтетический тест выше.
