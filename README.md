...
продукта (майлстоун M2) — аккаунты, дневник питания с AI-разбором еды по
тексту и фото, редактируемый черновик с уточняющими вопросами, итоги дня и
режим «скрыть калории». Без `ANTHROPIC_API_KEY` разбор работает через
